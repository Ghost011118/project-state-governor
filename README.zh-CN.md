...
- 保留已经核验的工作流 `COMPLETED`（已完成）和 `CANCELLED`（已取消）终态，直到其细节可以安全压缩为仍与决策相关的历史；
...
所有者和产品决策都不得越过适用法律、真实授权边界、不可豁免的安全约束及客观事实。Skill 会先核实所谓硬约束是否真实且适用，不会把惯例、偏好或猜测擅自提升为不可覆盖的约束。
